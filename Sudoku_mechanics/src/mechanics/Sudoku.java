...
public class Sudoku {
        ...
        for (int y = 0; y < 9; y++)
            for (int x = 0; x < 9; x++){
                if (cells[x][y].getNumber() == 0)
                    if (cells[x][y].getImNumber()[num-1] &&
                            ((y == secondY) || (x == secondX) ||
                        ...
                        if (cells[x][y].getImNumber()[num-1]) {
                            ...
                            for (int secondX = 0; secondX < 9; secondX++)
                                for (int secondY = 0; secondY < 9; secondY++) {
                                    ...
                                    if (cells[secondX][secondY].getNumber() == 0)
                                        if (cells[secondX][secondY].getImNumber()[num - 1] &&
                                                cells[x][y].getSquare() == cells[secondX][secondY].getSquare() &&
                                                (x != secondX || y != secondY))
                                            isPrintSquare = false;

                                    if (cells[secondX][y].getNumber() == 0)
                                        if (cells[secondX][y].getImNumber()[num - 1] && x != secondX)
                                            isPrintLine = false;

                                    if (cells[x][secondY].getNumber() == 0)
                                        if (cells[x][secondY].getImNumber()[num - 1] && y != secondY)
                                            isPrintColumn = false;
                                }
        ...
        for (int num = 1; num < 10; num++)
            for (int y_x = 0; y_x < 9; y_x++)
            {
                ...
                for (int x_y = 0; x_y < 9; x_y++) {
                    if (cells[x_y][y_x].getImNumber()[num - 1] && isEnterLine) {
                    ...
                    if (cells[y_x][x_y].getImNumber()[num - 1] && isEnterColumn) {
                ...
                if (isClearSquareLine || isClearSquareColumn){
                    for (int y = 0; y < 9; y++) {
                        if (y != y_x)
                            for (int x = 0; x < 9; x++){
                                if (isClearSquareLine && cells[x][y].getImNumber()[num-1] && cells[x][y].getSquare() == squareLine) {
                                    cells[x][y].setImNumber(num, true);
                                    isWorks = true;
                                }

                                if (isClearSquareColumn && cells[y][x].getImNumber()[num-1] && cells[y][x].getSquare() == squareColumn) {
    ...
    private boolean guidingImNumbers() {
        ...
        for (int num = 1; num < 10; num++)
            for (int square = 0; square < 9; square++) {
                ...
                for (int y = 0; y < 9; y++)
                    for (int x = 0; x < 9; x++) {
                        if (cells[x][y].getSquare() == square && cells[x][y].getImNumber()[num-1]){
                            if (y_potential == -1) {
                                isLine = true;
                                y_potential = y;
                            }
                        ...
                        if (cells[y][x].getSquare() == square && cells[y][x].getImNumber()[num-1]){
                            if (x_potential == -1) {
                                isColumn = true;
                                x_potential = y;
                            }
    ...
    //TODO
    public boolean nakedImNumbers(){
        boolean isWorks = false;

        for(int count = 2; count < 5; count++) {
            for (int y = 0; y < 9; y++) {
                for (int x = 0; x < 9; x++) {
                    if (cells[x][y].countOfImNumbers() <= count && cells[x][y].countOfImNumbers() != 0) {

                        Sudoku_cell control_cellLine = cells[x][y];
                        Sudoku_cell control_cellColumn = cells[x][y];
                        Sudoku_cell control_cellSquare = cells[x][y];

                        int checkCountLine = count - 1;
                        int checkCountColumn = count -1;
                        int checkCountSquare = count -1;
                        ...
                        for (int secondY = 0; secondY < 9; secondY++)
                            for (int secondX = 0; secondX < 9; secondX++){
                                if ( (secondY > y || (secondX > x && y == secondY)) &&
                                    (x == secondX || y == secondY || cells[x][y].getSquare() == cells[secondX][secondY].getSquare()) &&
                                    (cells[secondX][secondY].countOfImNumbers() <= count) ){
